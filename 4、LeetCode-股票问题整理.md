...
问题要求解的是 ”最后一天结束后用户能够获得的最大利润“，我们可以记录每一天结束后，用户已获得的利润。每天用户已获得的利润与 “用户当天的操作” 有关：

- 买入：手上持有股票，已获得的利润 = 前一天的利润 - 股票的价格
- 卖出：手上不持有股票，已获得的利润 = 前一天的利润 + 股票的价格
- 不买不买，股票持有状态不变，已获得的利润不变
...
2、“数组”：构造数组 $dp[i][2]$，$dp[i][0]$ 表示第 $i$ 天卖出股票后已获得的最大利润，$dp[i][1]$ 表示第 $i$ 天买入股票后已获得的最大利润；
